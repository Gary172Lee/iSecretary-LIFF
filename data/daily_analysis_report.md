...
> 報告狀態：**部分完成**｜產生時間：2026-08-24 14:25:10（Asia/Taipei）

## 1. 今日結論

**部分完成：必要資料未全部成功產生；以下依現有檔案分析。**
**今日資料尚未更新；以下使用最近一次成功資料。**
**今日無可直接買進標的。**
Raw候選 37 檔；舊路徑退役觀察 0 檔；新假設v2.1預先登記觀察 0 檔。
Full Validation已由使用者暫緩，不會自動執行，也不會耗用七年驗證的Actions額度。
Swing Buy 20D：6 檔合格；依 Extension Momentum 作第二層排序，v4.0 Gate 不變。
Swing Buy 20D：候選池 6；正式 Buy Now 0。正式名單以現價 Forward Entry Edge 為準。
Deep Recovery 3M：1 檔候選；屬早期復甦觀察池，不是BuyNow。

## 2. 資料日期、版本、工具讀取結果、P0安全與歷史來源稽核

- 報告版本：`daily_after_market_report_v5_deep_recovery_3m`
- 產生時間：2026-08-24 14:25:10（Asia/Taipei）
- latest_indicators：2026-08-24 14:24:59
- gate_diagnostics：未取得
- 資料為今日：否
- 策略與引擎版本相容：否
- Diagnostic版本：`未取得`
- 舊路徑Manifest來源稽核：是
- P0安全：是；歷史洩漏=0；每日洩漏=未載明
- GitHub Actions本機檔案讀取：
  - `data/latest_indicators.json`：成功
  - `data/gate_diagnostics.json`：失敗（file_not_found）
  - `data/failed_breakout_watchlist.json`：成功
  - `data/performance_summary.json`：略過／不存在（file_not_found）
  ...
  - `data/swing_performance_summary.json`：略過／不存在（file_not_found）
...
- 今日候選池：6；Buy Now：0；Wait Pullback：5；Reject：1。
**今日 Buy Now = 0 檔。寧可沒有標的，也不把已過度延伸的強勢股當成現在可追價。**
**Wait Pullback（股票可強，但現價不是好買點；不列正式買進）**
- 6669 緯穎｜**Wait Pullback**：現價進場優勢 49.74；剩餘空間 10.0%；Forward RR 1.43；Pullback參考 6297.50；原因：forward_entry_edge_score_below_80、forward_rr_below_1_8、traceable_headroom_below_12pct。
- 2603 長榮｜**Wait Pullback**：現價進場優勢 48.62；剩餘空間 10.0%；Forward RR 1.43；Pullback參考 230.25；原因：entry_quality_below_7、extension_from_sma20_above_12pct、forward_entry_edge_score_below_80、forward_rr_below_1_8。
- 6472 保瑞｜**Wait Pullback**：現價進場優勢 48.47；剩餘空間 10.0%；Forward RR 1.43；Pullback參考 431.00；原因：entry_quality_below_7、forward_entry_edge_score_below_80、forward_rr_below_1_8、pa_requires_pullback_only。
- 3008 大立光｜**Wait Pullback**：現價進場優勢 44.42；剩餘空間 10.0%；Forward RR 1.43；Pullback參考 4970.86；原因：entry_quality_below_7、extension_from_sma20_above_12pct、forward_entry_edge_score_below_80、forward_rr_below_1_8。
- 2324 仁寶｜**Wait Pullback**：現價進場優勢 41.48；剩餘空間 10.0%；Forward RR 1.43；Pullback參考 40.30；原因：entry_quality_below_7、forward_entry_edge_score_below_80、forward_rr_below_1_8、traceable_headroom_below_12pct。
- Forward Entry policy：`swing_forward_entry_edge_v1`。
- 驗證狀態：尚未累積足夠BuyNow完成樣本；以每次 BuyNow 當日現價固定為D0，驗證T+20報酬、MFE、勝率與先停損率。
...
- **6488 環球晶｜Deep Recovery 3M**：現價 1010.00；3M最高收盤 1515.00；距高點 -33.3%；回前高空間 +50.0%；自低點回升 +29.8%；Recovery ModerateUp；延伸動能 58.0／Moderate；RS改善 9.2；量能 0.94x；候選分 69.40／RecoveryEmerging；題材 `semiconductor`；基本面/消息面：待ChatGPT排程即時健康審核。
...
- 新假設候選：0
- 主要排除原因：
  - Tradeable Second Leg path 未通過：做多方向一致：27檔
  - Tradeable Second Leg path 未通過：型態與分數品質：27檔
  - Tradeable Second Leg path 未通過：進場確認：27檔
  - Tradeable Second Leg path 未通過：合法突破情境：27檔
  - Tradeable Second Leg path 未通過：突破跟隨：27檔
  - Tradeable Second Leg path 未通過：對手方被困：27檔
  - Tradeable Second Leg path 未通過：True Breakout 品質：27檔
  - Tradeable Second Leg path 未通過：可交易第二段：27檔

## 8. True Breakout／Follow-through／停損與no-add風險

- 候選資料中的no-add guardrail：34檔；停損執行阻擋：0檔。
- no-add=true時，任何pullback zone只可作`observation_only`，不得作買點或加碼點。
- **2201 裕隆｜不可買**：階段 `setup_observation`；類型 `tradeable_second_leg`；分數 47.75；RR 2.02；目標 8.42%；原因：P0 Shadow Mode：獨立掃描器兩條現行進場假設均已退役，待重新設計與正式驗證
- **2887 台新金｜不可買**：階段 `setup_observation`；類型 `tradeable_second_leg`；分數 未載明；RR 4.29；目標 未載明；原因：Tradeable Second Leg path 未通過：做多方向一致 | Tradeable Second Leg path 未通過：型態與分數品質 | Tradeable Second Leg path 未通過：進場確認 | Tradeable Second Leg path 未通過：目標報酬至少 10% | Tradeable Second Leg path 未通過：合法突破情境 | Tradeable Second Leg path 未通過：突破跟隨 | Tradeable Second Leg path 未通過：對手方被困 | Tradeable Second Leg path 未通過：True Breakout 品質 | Tradeable Second Leg path 未通過：可交易第二段
- **3443 創意｜不可買**：階段 `setup_observation`；類型 `tradeable_second_leg`；分數 未載明；RR 2.28；目標 未載明；原因：Tradeable Second Leg path 未通過：資料、流動性與滑價 | Tradeable Second Leg path 未通過：做多方向一致 | Tradeable Second Leg path 未通過：型態與分數品質 | Tradeable Second Leg path 未通過：進場確認 | Tradeable Second Leg path 未通過：目標報酬至少 10% | Tradeable Second Leg path 未通過：合法突破情境 | Tradeable Second Leg path 未通過：突破跟隨 | Tradeable Second Leg path 未通過：對手方被困 | Tradeable Second Leg path 未通過：True Breakout 品質 | Tradeable Second Leg path 未通過：可交易第二段
...
- **1504 東元｜不可買**：階段 `setup_observation`；類型 `tradeable_second_leg`；分數 未載明；RR 6.50；目標 未載明；原因：Tradeable Second Leg path 未通過：做多方向一致 | Tradeable Second Leg path 未通過：型態與分數品質 | Tradeable Second Leg path 未通過：進場確認 | Tradeable Second Leg path 未通過：目標報酬至少 10% | Tradeable Second Leg path 未通過：合法突破情境 | Tradeable Second Leg path 未通過：突破跟隨 | Tradeable Second Leg path 未通過：對手方被困 | Tradeable Second Leg path 未通過：True Breakout 品質 | Tradeable Second Leg path 未通過：可交易第二段
...
- Performance完成樣本：0；正式Direct Buy完成樣本：0。
- 被拒絕候選整體勝率：未載明；T+5平均報酬：未載明；MFE：未載明；MAE：未載明。
